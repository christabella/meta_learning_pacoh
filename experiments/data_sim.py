...
class PhysionetDataset(MetaDataset):
    def __init__(
        self, random_state=None, variable_id=0, dtype=np.float32, physionet_dir=None
    ):
    ...
    def generate_meta_test_data(
        self, n_tasks, n_samples_context=24, n_samples_test=-1, variable_id=0
    ):
        """
        Samples n_tasks patients and returns measurements from the variable
        with the ID variable_id. n_samples defines in this case the cut-off
...
class MNISTRegressionDataset(MetaDataset):
    ...
    def generate_meta_test_data(self, n_tasks, n_samples_context, n_samples_test=-1):
        ...
        test_indices = self.random_state.choice(
            self.test_images.shape[0], size=n_tasks, replace=False
        )
    ...
    def _image_to_context_transform(self, image, num_context_points, context_mask=None):
        ...
        return (
            np.array(context_indices, dtype=self.dtype),
            np.array(context_values, dtype=self.dtype),
            np.array(test_indices, dtype=self.dtype),
            np.array(test_values, dtype=self.dtype),
        )
...
class SinusoidDataset(MetaDataset):
    ...
    def generate_meta_test_data(self, n_tasks, n_samples_context, n_samples_test):
        ...
        for i in range(n_tasks):
            ...
            X = self.random_state.uniform(
                self.x_low, self.x_high, size=(n_samples_context + n_samples_test, 1)
            )
    ...
    def _sample_sinusoid(self):
        ...
        x_shift = self.random_state.normal(
            loc=self.x_shift_mean, scale=self.x_shift_std
        )
        y_shift = self.random_state.normal(
            loc=self.y_shift_mean, scale=self.y_shift_std
        )
        ...
        return (
            lambda x: slope * x + amplitude * np.sin(period * (x - x_shift)) + y_shift
        )
...
class SinusoidNonstationaryDataset(MetaDataset):
    ...
    def _sample_fun(self):
        ...
        return (
            lambda x: mean(x)
            + np.sin(freq(x) * x)
            + self.random_state.normal(loc=0, scale=self.noise_std, size=x.shape)
        )
...
class CauchyDataset(MetaDataset):
    ...
    def _gp_fun_from_prior(self, X):
        ...
        f = (
            self._mean(X)
            + np.dot(L, self.random_state.normal(scale=0.2, size=(n, 1))).flatten()
        )
        y = f + self.random_state.normal(scale=self.noise_std, size=f.shape)
...
class SwissfelDataset(MetaDataset):
    ...
    def generate_meta_train_data(self, n_tasks=5, n_samples=200):
        assert n_tasks == len(self.run_specs_train), "number of tasks must be %i" % len(
            self.run_specs_train
        )
        ...
        assert n_samples <= max_n_samples, (
            "only %i number of samples available" % max_n_samples
        )

        meta_train_tuples = [
            (X[:n_samples], Y[:n_samples]) for X, Y in meta_train_tuples
        ]

        return meta_train_tuples

    def generate_meta_test_data(
        self, n_tasks=None, n_samples_context=200, n_samples_test=400
    ):
        if n_tasks is None:
            n_tasks = len(self.run_specs_test)

        assert n_tasks == len(self.run_specs_test), "number of tasks must be %i" % len(
            self.run_specs_test
        )
        meta_test_tuples = self._load_meta_dataset(train=False)

        max_n_samples = min([X.shape[0] for X, _ in meta_test_tuples])
        assert n_samples_context + n_samples_test <= max_n_samples, (
            "only %i number of samples available" % max_n_samples
        )
...
def provide_data(dataset, seed=28, n_train_tasks=None, n_samples=None):
    ...
    if "sin-nonstat" in dataset:
        ...
        dataset = PhysionetDataset(
            random_state=np.random.RandomState(seed + 1), variable_id=variable_id
        )
        ...
        data_train = dataset.generate_meta_train_data(
            n_tasks=n_train_tasks, n_samples=n_train_samples
        )

        data_test_valid = dataset.generate_meta_test_data(
            n_samples_context=n_context_samples, n_samples_test=N_TEST_SAMPLES
        )
    ...
    data_test_valid = dataset.generate_meta_test_data(
        n_tasks=N_TEST_TASKS + N_VALID_TASKS,
