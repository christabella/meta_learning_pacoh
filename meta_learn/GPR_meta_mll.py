...
class GPRegressionMetaLearned(RegressionModelMetaLearned):
    def __init__(self,
        ...
        assert learning_mode in [
            'learn_mean', 'learn_kernel', 'both', 'vanilla'
        ]
        ...
        self._setup_gp_prior(mean_module, covar_module, learning_mode,
                             feature_dim, mean_nn_layers, kernel_nn_layers)
        self.likelihood = gpytorch.likelihoods.GaussianLikelihood(
            noise_constraint=gpytorch.likelihoods.noise_models.GreaterThan(
                1e-3)).to(device)
        ...
        for train_x, train_y in meta_train_data:
            ...
            task_dict['mll_fn'] = gpytorch.mlls.ExactMarginalLogLikelihood(
                self.likelihood, task_dict['model']).to(device)
    ...
    def meta_fit(self,
        ...
        Args:
            ...
            log_period: (int) number of steps after which to print (validation) stats
            ...
            for itr in range(1, n_iter + 1):
                ...
                if itr == 1 or itr % log_period == 0:
                    ...
                    message = 'Iter %d/%d - Loss: %.6f - Time %.2f sec' % (
                        itr, self.num_iter_fit, avg_loss.item(), duration)
    ...
    def _setup_gp_prior(self, mean_module, covar_module, learning_mode,
                        feature_dim, mean_nn_layers, kernel_nn_layers):
        ...
        if mean_module == 'NN':
            ...
            self.nn_mean_fn = NeuralNetwork(
                input_dim=self.input_dim,
                output_dim=1,
                layer_sizes=mean_nn_layers).to(device)
            self.shared_parameters.append({
                'params':
                self.nn_mean_fn.parameters(),
                'lr':
                self.lr_params,
                'weight_decay':
                self.weight_decay
        ...
        if learning_mode in ["learn_kernel", "both"]:
            self.shared_parameters.append({
                'params':
                self.covar_module.hyperparameters(),
                'lr':
                self.lr_params
            })

        if learning_mode in ["learn_mean", "both"
                             ] and self.mean_module is not None:
            self.shared_parameters.append({
                'params':
                self.mean_module.hyperparameters(),
                'lr':
                self.lr_params
            })
...
if __name__ == "__main__":
    ...
    for weight_decay in [0.8, 0.5, 0.4, 0.3, 0.2, 0.1]:
        gp_model = GPRegressionMetaLearned(meta_train_data,
                                           num_iter_fit=20000,
        ...
        for i in range(1):
            gp_model.meta_fit(valid_tuples=meta_test_data,
                              log_period=1000,
                              n_iter=20000)
            itrs += 20000
            ...
            pred_mean, pred_std = gp_model.predict(x_context, t_context,
                                                   x_plot)
            ucb, lcb = gp_model.confidence_intervals(x_context,
                                                     t_context,
                                                     x_plot,
                                                     confidence=0.9)
            ...
            plt.title('GPR meta mll (weight-decay =  %.4f) itrs = %i' %
                      (weight_decay, itrs))
            plt.show()
